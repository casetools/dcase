...
import edu.casetools.dcase.extensions.io.mdiag2uppaal.mnta.locations.NTRLocations;
...
public class EventGenerator extends AbstractTemplateGenerator {
	...
	auxiliarTransition.setSync(new Synchronization(EVENTS_CHANNEL + id + "+1]", SyncType.INITIATOR, 221, -68));
	...
	auxiliarTransition.setSync(new Synchronization(EVENTS_CHANNEL + id + "]", SyncType.RECEIVER, -34, 8));
	auxiliarTransition.addNail(new Nail(-51, 17));

	auxiliarTransition = new Transition(m, next, initial);
	auxiliarTransition.setSync(new Synchronization(EVENTS_CHANNEL + id + "]", SyncType.INITIATOR, -450, 25));
	...
	auxiliarTransition.setSync(new Synchronization(EVENTS_CHANNEL + id + "]", SyncType.INITIATOR, -170, -68));

	auxiliarTransition = new Transition(m, triggered, waiting);
	auxiliarTransition.setSync(new Synchronization(EVENTS_CHANNEL + id + "]", SyncType.INITIATOR, 0, -68));

	auxiliarTransition = new Transition(m, initial, notTriggered);
	auxiliarTransition.setGuard(new Guard(invertedRule, -280, -179));
	auxiliarTransition.setSync(new Synchronization(EVENTS_CHANNEL + id + "]", SyncType.RECEIVER, -280, -195));
	...
	auxiliarTransition.setSync(new Synchronization(EVENTS_CHANNEL + id + "]", SyncType.RECEIVER, 34, -204));
