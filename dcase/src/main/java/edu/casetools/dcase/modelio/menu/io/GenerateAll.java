...
public class GenerateAll extends DefaultModuleCommandHandler {
	...
	String selectedPath = dialog.open().concat("\\").concat(projectName);
	
	String path = selectedPath.concat(mobileFolder);
	MobileCodeGenerator mobileGenerator = new MobileCodeGenerator();
	mobileGenerator.generateTemplates(path);
	path = selectedPath.concat(stationaryFolder);
