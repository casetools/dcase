 ...
 * 
 */
package edu.casetools.dcase.modelio.diagrams.acqmod.tools.elements;
 ...
 */
public class RDFModellingRuleTool extends ElementTool {
	...
	String name = I18nMessageService.getString("Names.RDFModellingRule");
	...
	Class auxiliarElement = ElementUtils.getInstance().createClass(DCaseModule.getInstance(), adaptElement(element), session, name,
			...
			return ElementUtils.getInstance().addStereotype(DCasePeerModule.MODULE_NAME, auxiliarElement, DCaseStereotypes.STEREOTYPE_RDF_MODELLING_RULE);
    }
	...
	if ((null != graph) && (!graph.isEmpty()) && (graph.get(0) instanceof IDiagramNode)) {
	    ...
	    dnode.setProperty("FILLCOLOR", RCaseColours.GREEN0);
