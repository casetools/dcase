...
public class NTRPropertyPage implements IPropertyContent {

    private static final Logger LOGGER = Logger.getLogger(NTRPropertyPage.class.getName());
	...
	try {
		...
		PropertiesUtils.getInstance().findAndAddValue(DCaseModule.getInstance(), DCasePeerModule.MODULE_NAME,
			DCaseProperties.PROPERTY_NTR_PLATFORM, value, element);
		break;	
	    default:
		break;
	    }
	...
	// ID
	property = element.getTagValue(DCasePeerModule.MODULE_NAME, DCaseProperties.PROPERTY_NTR_ID);
	table.addConsultProperty(I18nMessageService.getString("Ui.State.Property.TagId"), property);
	
	// TagPlatformValue
	updateNTRPlatform((ModelElement) element);
	property = element.getTagValue(DCasePeerModule.MODULE_NAME, DCaseProperties.PROPERTY_NTR_PLATFORM);
	table.addConsultProperty(I18nMessageService.getString("Ui.Platform"), property);

    }
    
    public static void updateNTRPlatform(ModelElement element){
    	ContextStatePropertyPage.putTagValue(element, DCaseProperties.PROPERTY_NTR_PLATFORM, getNTRPlatformType(element));
    }

	public static String getNTRPlatformType(ModelElement element) {
