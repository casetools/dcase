 ...
 */
public class InternalTimeStateTool extends ElementTool {
	...
	String name = I18nMessageService.getString("Names.InternalTimeState");

	return ElementUtils.getInstance().createClass(DCaseModule.getInstance(), adaptElement(element), session, name,
		DCaseStereotypes.STEREOTYPE_INTERNAL_TIME_STATE);
    }
	...
	if ((null != graph) && (!graph.isEmpty()) && (graph.get(0) instanceof IDiagramNode)) {
	    ...
	    dnode.setProperty("FILLCOLOR", DCaseColours.WHITE);
